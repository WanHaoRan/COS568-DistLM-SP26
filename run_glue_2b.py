...
def sync_gradients(model, world_size, rank):
    """
    Synchronize gradients across all workers using all_reduce (Task 2b).
    Sums gradients on every rank then divides by world_size to get the mean.
    """
...
def train(args, train_dataset, model, tokenizer):
    ...
    if args.profile:
        ...
        trace_addr = os.path.join(args.output_dir, "chrome_trace_part2b_rank{}.json".format(rank))
        ...
        for epoch in train_iterator:
            ...
            for step, batch in enumerate(epoch_iterator):
                ...
                if ((step + 1) % args.gradient_accumulation_steps == 0) or ((step + 1) == len(train_dataloader)):
                    # Task 2b: sync gradients via all_reduce before clip/step
                    if args.master_ip is not None:
    ...
    os.makedirs(args.output_dir, exist_ok=True)
    avg_time_path = os.path.join(args.output_dir, "avg_time_part2b_rank{}.txt".format(rank))
    if len(iter_times) > 1:
        avg_time_per_iter = sum(iter_times[1:]) / (len(iter_times) - 1)
        logger.info("[Task 2(b)] Rank %s: average time per iteration (excluding first): %.4f sec (%d iterations)",
                    rank, avg_time_per_iter, len(iter_times) - 1)
        with open(avg_time_path, "w") as f:
            f.write("rank\tavg_time_per_iter_sec\tnum_iterations\n")
            f.write("%d\t%.6f\t%d\n" % (rank, avg_time_per_iter, len(iter_times) - 1))
        logger.info("[Task 2(b)] Rank %s: avg time written to %s", rank, avg_time_path)

    # Log loss curve for this node (each rank writes its own file and plot)
    loss_curve_path = os.path.join(args.output_dir, "loss_curve_part2b_rank{}.txt".format(rank))
    with open(loss_curve_path, "w") as f:
        f.write("step\tloss\n")
        for s, l in loss_curve:
            f.write("%d\t%.6f\n" % (s, l))
    logger.info("[Task 2(b)] Rank %s: loss curve written to %s", rank, loss_curve_path)
    ...
    if loss_curve and _HAS_MATPLOTLIB:
        ...
        plot_path = os.path.join(args.output_dir, "loss_curve_part2b_rank{}.png".format(rank))
        plt.savefig(plot_path, dpi=150, bbox_inches="tight")
        plt.close()
        logger.info("[Task 2(b)] Rank %s: loss curve plot saved to %s", rank, plot_path)
